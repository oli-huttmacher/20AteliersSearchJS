const parentDuRendu = document.getElementById("render") as HTMLElement;
const promptInput = document.getElementById("inputPrompt") as HTMLInputElement;
const galery = document.querySelector(".galery") as HTMLElement;
...
//le timer---------------------------------------------
let timer: number | undefined; // pour stocker le setTimeout
//resultat du tableau
let resultInput: string = ""; //le string de la recherche
//listener de l'input capture le prompt et le met dans la variable resultinput
promptInput.addEventListener("input", (e) => {
  const valueBrut = e.target as HTMLInputElement; //<input>
  ...
  const fruitTrouve = tabFruits.find(
    (fruit) => fruit.toLowerCase() === resultInput.toLowerCase()
  );
...
function nettoyageInputValue(valuePrompt: string): string {
...
function filtrage(tab: string[], promptPropre: string): string[] {
  if (promptPropre === "") {
...
function discover(fruits: string) {
...
function retard(term: string, delay: number) {
...
function nombreAleatoire0a10(): number {
